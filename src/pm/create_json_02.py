...
with open('data/02.csv', 'r') as f:
    ...
    for row in reader:
        ...
        img_proccess_flg = True

        # A-KAZE検出器の生成
        akaze = cv2.AKAZE_create() 

        files = glob.glob("data/temp_all/2/2-"+index+"*.tif")
        ...
        for file in sorted(files):
            ...
            if img_proccess_flg:
                # 画像１
                img1 = cv2.imread("data/target_all/2/input/"+page+".jpg")
                height, width, channels = img1.shape[:3]

                # 特徴量の検出と特徴量ベクトルの計算
                kp1, des1 = akaze.detectAndCompute(img1, None)

                img_proccess_flg = False
